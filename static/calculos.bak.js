...
function _invalidateCache() {
  ...
  CACHE.dataInvalidation    = Date.now();
}
...
/**
 * Garante que valor seja um número válido. Retorna fallback se inválido.
 */
...
const fmt = {
  ...
  brl(v) {
    ...
    const n = _toNumber(v);
    if (n === 0 && v !== 0 && v !== '0') return '—'; // Retorna '—' apenas se foi fallback
  ...
  /**
   * Formata um número com casas decimais customizadas.
   */
  custom(v, decimals = 2) {
    if (v == null) return '—';
    const n = _toNumber(v);
