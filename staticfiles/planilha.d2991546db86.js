...
function initNegativeValueMonitoring() {
  const checkAndApplyNegativeClass = (el) => {
  ...
  document.querySelectorAll('input[type="number"]').forEach(input => {
    checkAndApplyNegativeClass(input);
    input.addEventListener('change', () => checkAndApplyNegativeClass(input));
    input.addEventListener('input', () => checkAndApplyNegativeClass(input));
  });

  // Monitor all text inputs that might contain numbers
  document.querySelectorAll('input[type="text"][data-is-number]').forEach(input => {
    checkAndApplyNegativeClass(input);
    input.addEventListener('change', () => checkAndApplyNegativeClass(input));
    input.addEventListener('input', () => checkAndApplyNegativeClass(input));
  });

  // Also check table cells and spans with numerical content
  const checkCell = (el) => {
    const text = el.textContent.trim();
    const val = parseFloat(text.replace(/[R$\s,.]/g, '').replace(',', '.'));
    if (!isNaN(val) && val < 0) {
      el.classList.add('is-negative');
    } else {
      el.classList.remove('is-negative');
