...
function applyNegativeValueStyling() {
  ...
  const isAlwaysRedField = (text) => {
    // Never apply "always red" to "LUCRO LÍQUIDO" - it should follow normal rules
    if (text.toUpperCase().includes('LUCRO LÍQUIDO') || text.toUpperCase().includes('LUCRO LIQUIDO')) {
      return false;
    }
  ...
  document.querySelectorAll('input[type="number"]').forEach(input => {
    ...
    const isLucroLiquido = parentText.toUpperCase().includes('LUCRO LÍQUIDO') || 
                           parentText.toUpperCase().includes('LUCRO LIQUIDO');
    
    if (!isNaN(val)) {
      if (isLucroLiquido) {
        // Lucro Líquido: green if positive, red if negative
        if (val < 0) {
    ...
    input.addEventListener('input', function() {
      ...
      const pText = this.parentElement?.textContent || '';
      const isLucro = pText.toUpperCase().includes('LUCRO LÍQUIDO') || 
                      pText.toUpperCase().includes('LUCRO LIQUIDO');
      ...
      if (!isNaN(v)) {
        if (isLucro) {
          ...
          const pText2 = this.parentElement?.textContent || '';
          const isCurrencyField = pText2.includes('R$') || 
