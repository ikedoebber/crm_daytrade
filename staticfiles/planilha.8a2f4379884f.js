/* ================================================
   ...
   calculos.js DEVE ser carregado ANTES deste arquivo.
   ================================================ */
...
const STATE = {
  month:        '',
  config:       {},
  projecao:     [],   // [{dia, realizado, custo_op, imposto_retido, capital_final}]
  operacoes:    [],
  diario:       [],
  regras:       [],
  capital_atual:{},
...
async function loadAll(month) {
  try {
    ...
    // capital-atual separado para não bloquear se o endpoint ainda não existir
    let capitalAtual = {};
    try {
      capitalAtual = await API.get(`/api/projecao/capital-atual/?month=${month}`);
    } catch { /* silencioso */ }
...
function renderAll() {
  ...
  renderDiario();
  renderRegras();
           // calculos.js — escreve todos os campos calculados
...
function renderDashboard() {
  ...
  set('totalDiasDisp',   dias);
  set('diasPercorridos', diasFeitos);

  if (STATE.capital_atual?.capital_final !== undefined) {
    set('capitalAtualFromDB', fmt.brl(STATE.capital_atual.capital_final));
  }
  ...
  set('sumLoss',  losses);
  set('sumZero',  zeros);
  set('sumTotal', ops.length);
  set('sumWR',    (gains + losses) > 0 ? ((gains / (gains + losses)) * 100).toFixed(1) + '%' : '—');

  set('dashMetaAprovacao',        fmt.brl(c.plano_meta_aprovacao));
  set('dashPerdaMaxTotal',        fmt.brl(c.plano_perda_max_total));
  set('dashPerdaDiariaAprovacao', fmt.brl(c.plano_perda_diaria_aprovacao));
}

// ─── PROJEÇÃO DIÁRIA ──────────────────────────────
// "Realizado" é somente leitura: vem da soma das operações via signal do backend.
// Custo Operacional e Imposto Retido são editáveis e refletem nos Relatórios.
function renderProjecaoTable() {
  ...
  for (let d = 1; d <= dias; d++) {
    ...
    // realizado vem do backend (soma das operações do dia) — não editável
    const realizado   = dadoDia.realizado      ?? null;
    ...
    if (realizado !== null && realizado !== '') {
      ...
      const retornoPct = banca > 0 ? ((capitalFinal - banca) / banca * 100) : 0;
      retorno          = retornoPct.toFixed(2) + '%';
      retornoClass     = retornoPct >= 0 ? 'val-positive' : 'val-negative';
    ...
    const realValor = (realizado !== null && realizado !== '')
      ? fmt.brl(parseFloat(realizado)) : '—';
    ...
    tr.innerHTML = `
      ...
      <!-- REALIZADO: somente leitura, calculado pelas operações -->
      <td class="${realClass}" style="font-family:var(--mono);font-weight:600;white-space:nowrap">
        ${realValor}
        <span style="display:block;font-size:9px;color:var(--muted);font-weight:400;margin-top:1px">▸ ops</span>
      </td>
      ...
      <td>${pctMeta}</td>
      <td style="color:var(--neon2);font-weight:600">
        ${(realizado !== null && realizado !== '') ? fmt.brl(capitalFinal) : '—'}
      </td>
      <td class="${retornoClass}">${retorno}</td>
    `;
    tbody.appendChild(tr);
  }

  // Atualiza campos calculados sempre que a tabela é redesenhada
  // (custo/imposto podem ter mudado via input antes do save)
  _bindProjecaoInputs();
}

/** Vincula eventos nos inputs de custo/imposto para atualizar relatórios em tempo real. */
function _bindProjecaoInputs() {
  document.querySelectorAll('.proj-custo, .proj-imposto').forEach(inp => {
    inp.addEventListener('input', () => {
      const dia   = parseInt(inp.dataset.dia);
      const entry = STATE.projecao.find(p => p.dia === dia);
      if (entry) {
        if (inp.classList.contains('proj-custo'))    entry.custo_op       = parseFloat(inp.value) || 0;
        if (inp.classList.contains('proj-imposto'))  entry.imposto_retido = parseFloat(inp.value) || 0;
      } else {
        // Dia ainda não existe no STATE — cria entrada temporária
        const newEntry = { dia, realizado: null, custo_op: 0, imposto_retido: 0 };
        if (inp.classList.contains('proj-custo'))    newEntry.custo_op       = parseFloat(inp.value) || 0;
        if (inp.classList.contains('proj-imposto'))  newEntry.imposto_retido = parseFloat(inp.value) || 0;
        STATE.projecao.push(newEntry);
      }
       // reflete em Relatórios instantaneamente
    });
  });
}
...
function buildOpsRow(op, idx) {
  ...
  tr.querySelectorAll(
    'input[name="entrada"], input[name="saida"], input[name="contratos"], input[name="valor_ponto"], select[name="ativo"]'
  ).forEach(inp => inp.addEventListener('change', () => autoCalcRow(tr)));
...
function autoCalcRow(tr) {
  ...
  if (entrada && saida) {
    ...
    tr.querySelector('[name="status"]').value = resultado > 0 ? 'GAIN' : resultado < 0 ? 'LOSS' : 'ZERADA';
  }
}
...
function renderCharts() {
  ...
  for (let d = 1; d <= dias; d++) {
    ...
    if (pDia && pDia.realizado !== null && pDia.realizado !== '') {
      cap += (parseFloat(pDia.realizado)||0) - (parseFloat(pDia.custo_op)||0) - (parseFloat(pDia.imposto_retido)||0);
  ...
  buildBarChart('chartResultadoDetalhados', labels, [
    { label: 'Resultado R$', data: resultDia, backgroundColor: resultDia.map(v=>(v??0)>=0?'rgba(34,217,135,0.5)':'rgba(255,76,106,0.5)') },
  ]);
...
function buildLineChart(id, labels, datasets) {
  ...
  if (!ctx) return;
  datasets.forEach(ds => {
    ds.tension = ds.tension ?? 0.3; ds.pointRadius = ds.pointRadius ?? 3;
    ds.pointHoverRadius = 5; ds.borderWidth = ds.borderWidth ?? 2;
  });
...
async function saveDiario() {
  ...
  } catch { setStatus('saveDiarioStatus', 'Erro ao salvar.', 'err'); }
}
...
function renderRegras() {
  ...
  STATE.regras.forEach((r, i) => {
    ...
    `;
    div.querySelector('.regra-del').addEventListener('click', () => { STATE.regras.splice(i, 1); renderRegras(); });
    div.querySelector('input').addEventListener('input', e => { STATE.regras[i].texto = e.target.value; });
    container.appendChild(div);
  });
}
...
async function saveRegras() {
  ...
  STATE.regras = await API.get('/api/regras/');
  renderRegras();
}
...
async function saveConfig(statusId) {
  ...
  try {
    const result = await API.post('/api/config/upsert/', getConfigFromInputs());
    STATE.config  = result;
    renderDashboard();
    renderProjecaoTable();
  ...
  } catch (e) { console.error(e); setStatus(statusId, 'Erro!', 'err'); }
}

async function saveProjecao() {
  setStatus('saveProjecaoStatus', 'Salvando...');
  try {
    // Envia apenas custo_op e imposto_retido — realizado é gerenciado pelo backend
    const items = Array.from(document.querySelectorAll('#projecaoBody tr')).map(tr => ({
      month:          STATE.month,
      dia:            parseInt(tr.dataset.dia),
      custo_op:       parseFloat(tr.querySelector('.proj-custo')?.value)   || 0,
      imposto_retido: parseFloat(tr.querySelector('.proj-imposto')?.value) || 0,
    }));
    ...
    renderProjecaoTable();
    renderCharts();
             // reflete novos custos/impostos nos Relatórios
    applyNegativeValueStyling();
    setStatus('saveProjecaoStatus', '✓ Salvo!', 'ok');
  } catch (e) { console.error(e); setStatus('saveProjecaoStatus', 'Erro!', 'err'); }
}
...
async function saveOperacoes() {
  ...
  try {
    ...
    // bulk-sync deleta e recria; o signal do backend recalcula ProjecaoDia.realizado
    const result = await API.post('/api/operacoes/bulk-sync/', { month: STATE.month, operacoes: ops });
    STATE.operacoes = result;

    // Recarrega projeção para pegar o "realizado" atualizado pelo signal
    STATE.projecao = await API.get(`/api/projecao/?month=${STATE.month}`);
  ...
  } catch (e) { console.error(e); setStatus('saveOpsStatus', 'Erro!', 'err'); }
}
...
async function savePlano() {
  ...
  } catch { setStatus('savePlanoStatus', 'Erro!', 'err'); }
}
...
function applyNegativeValueStyling() {
  ...
  const applyClass = (el, val) => {
    if (val < 0)      { el.classList.add('is-negative'); el.classList.remove('is-positive-currency'); }
    else if (val > 0) { el.classList.add('is-positive-currency'); el.classList.remove('is-negative'); }
    else              { el.classList.remove('is-negative','is-positive-currency'); }
  };
  document.querySelectorAll('input[type="number"]').forEach(input => {
    const val = parseFloat(input.value);
    if (!isNaN(val)) applyClass(input, val);
    input.addEventListener('input', function () { const v=parseFloat(this.value); if(!isNaN(v)) applyClass(this,v); });
  });
  document.querySelectorAll('td, div, span').forEach(el => {
    ...
    if (isAlwaysRed(pText))         applyClass(el, -1);
    else if (val < 0)               applyClass(el, val);
    else if (val > 0 && text.includes('R$')) applyClass(el, val);
  });
}
...
// ─── EVENTOS ──────────────────────────────────────
function bindEvents() {
  document.getElementById('monthSelect')?.addEventListener('change', e => { STATE.month=e.target.value; loadAll(STATE.month); });
  ['cfg_banca_inicial','cfg_objetivo_diario','cfg_dias_uteis'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', () => {
      STATE.config = { ...STATE.config, ...getConfigFromInputs() };
      renderDashboard(); renderProjecaoTable(); renderCharts(); 
    });
  });
  ...
  document.getElementById('plano_diario')?.addEventListener('keydown', e => { if(e.ctrlKey&&e.key==='Enter') saveDiario(); });
