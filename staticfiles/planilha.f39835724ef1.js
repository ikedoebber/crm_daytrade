...
const CONFIG_DEFAULTS = {
  ...
  objetivo_diario:              350,
  dias_uteis:                   18,
  plano_meta_aprovacao:         0,
  plano_perda_max_total:        0,
  plano_perda_diaria_aprovacao: 0,
  plano_capital:                0,
  plano_meta:                   0,
  plano_maxentradas:            5,
  plano_stop:                   0,
...
function fillConfigInputs() {
  ...
  fields.forEach(f => {
    ...
    if (!el) return;
    const val = c[f];
    // Usa o valor do backend se existir e não for zero/vazio, senão aplica o default
...
function renderProjecaoTable() {
  ...
  for (let d = 1; d <= dias; d++) {
    ...
    if (realizado !== '' && realizado !== null) {
      ...
      // Propaga o capital acumulado para dias sem operação
    ...
    // Capital Final: mostra valor acumulado mesmo sem realizado (desde que algum dia já tenha sido operado)
    const capitalFinalCell = (realizado !== '' && realizado !== null)
      ? fmt.brl(capitalFinal)
      : algumRealizado
        ? `<span style="opacity:0.5">${fmt.brl(capitalFinal)}</span>`
        : '—';
